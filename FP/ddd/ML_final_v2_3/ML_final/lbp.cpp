...
int getPatternfloat(cv::Mat patch, int x, int y){
	...
	int flag;
	int map[256];
	float origin = patch.at<float>(i,j);
	if(patch.at<float>(i-1, j-1) > origin) temp++;
	temp << 1;
	if(patch.at<float>(i-1, j) > origin) temp++;
	temp << 1;
	if(patch.at<float>(i-1, j+1) > origin) temp++;
	temp << 1;
	if(patch.at<float>(i, j+1) > origin) temp++;
	temp << 1;
	if(patch.at<float>(i+1, j+1) > origin) temp++;
	temp << 1;
	if(patch.at<float>(i+1, j) > origin) temp++;
